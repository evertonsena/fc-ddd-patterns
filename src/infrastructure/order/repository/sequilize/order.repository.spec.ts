...
describe("Order repository test", () => {
  ...
  it("should update a order", async () => {
    ...
    const customerNew = new Customer("987", "Customer New");
    ...
    const orderModelChange = await OrderModel.findOne({
      where: { id: order.id },
      include: ["items"],
    });

    expect(orderModelChange.toJSON()).toStrictEqual({
      id: "123",
  ...
  it("should find all orders", async () => {
    ...
    const allOrdersFind = await orderRepository.findAll();
    const allOrders = [order1, order2, order3];

    expect(allOrdersFind).toEqual(allOrders);    
